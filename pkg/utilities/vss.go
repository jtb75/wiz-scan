...
func CreateVSSSnapshot(drive string) (string, string, error) {
	...
	if err != nil {
		...
		logrus.Debugf("Created VSS snapshot for %s", drive)
	}
...
func extractShadowCopyID(output string) (string, error) {
	...
	if err := scanner.Err(); err != nil {
		logrus.Errorf("Error reading output for Shadow Copy ID: %v", err)
		return "", err
	}
...
func executeCommandAndGetOutput(command string, args ...string) (string, error) {
	...
	if err != nil {
		logrus.Errorf("Command execution failed: %s %v", command, err)
		return "", err
	}
...
func mountSnapshot(drive, shadowCopyVolume string) (string, error) {
	...
	if _, err := executeCommandAndGetOutput("cmd", "/C", mountCmd); err != nil {
		...
		logrus.Debugf("Mounted VSS Snapshot at %s", mountPath)
	}
...
func RemoveVSSSnapshot(mountedPath string, shadowCopyID string) error {
	...
	if err := exec.Command("cmd", "/C", removeMountCmd).Run(); err != nil {
		logrus.Errorf("Failed to remove VSS mount for %s: %v", mountedPath, err)
		return err
	} else {
		logrus.Debugf("Removed mounted VSS snapshot at %s", mountedPath)
	}
	...
	if err := exec.Command("cmd", "/C", deleteSnapshotCmd).Run(); err != nil {
		logrus.Errorf("Failed to delete VSS snapshot for %s: %v", shadowCopyID, err)
		return err
	} else {
		logrus.Debugf("Removed VSS snapshot id: %s", shadowCopyID)
	}
