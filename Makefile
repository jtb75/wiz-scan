...
linux-amd64:
	...
	@GOOS=linux GOARCH=amd64 go build $(LDFLAGS) -o $(GOBIN)/$(BINARY_LINUX_AMD64) $(GOBASE)/cmd/wiz-scan
	@echo "  >  Binary $(BINARY_LINUX_AMD64) created at $(GOBIN)"

# Build binary for macOS ARM64
macos-arm64:
	@echo "  >  Building binary for macOS ARM64..."
	@GOOS=darwin GOARCH=arm64 go build $(LDFLAGS) -o $(GOBIN)/$(BINARY_MACOS_ARM64) $(GOBASE)/cmd/wiz-scan
	@echo "  >  Binary $(BINARY_MACOS_ARM64) created at $(GOBIN)"

# Build binary for Windows AMD64
windows-amd64:
	@echo "  >  Building binary for Windows AMD64..."
	@GOOS=windows GOARCH=amd64 go build $(LDFLAGS) -o $(GOBIN)/$(BINARY_WINDOWS_AMD64) $(GOBASE)/cmd/wiz-scan
